...
export const UserProvider = ({ children }) => {
  ...
  const user = queryResult.data?.usersdata || [];
  const userData = user?.filter((item) => item.status.includes('active'));
  const inuserData = user?.filter((item) => item.status.includes('inactive'));

  const userCountByOffice = queryResult.data?.userbyOffice || [];
  const userCounts = queryResult.data?.userCounts || [];
  console.log('0000000000000000000000', userCountByOffice)
