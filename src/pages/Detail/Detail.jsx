...
import CreateUpdateAsset from './CreateUpdateDetail';
...
function Detail() {
    ...
    const name = "Asset registered under:---";
    const { userDetail, logoutUser } = React.useContext(AuthContext);
    const { accountDetail, updateUser, refetchAccount } = React.useContext(UserContext);
    ...
    const {
      ...
      const filtered = assetData?.filter(
        (asset) => asset.currentUser.email === accountDetail?.email
        );        
      
     
        React.useEffect(() => {
          const filteredAssets = assetData?.filter(
            (asset) => asset.currentUser.email === accountDetail && accountDetail?.email
            );        
        ...
        const columns = [
            ...
            return (
                <> 
                {/* {JSON.stringify(accountDetail.email) }
