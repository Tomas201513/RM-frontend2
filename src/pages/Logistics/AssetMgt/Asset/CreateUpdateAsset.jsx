...
function CreateUpdateAsset({  selectedData, editable, setEditable, name  }) {
  const {userDetail} = useContext(AuthContext);
    ...
    const {
      ...
      setSelectedData,
      setCreateOpen, 
      deleteAsset, warn, SetWarn } = useContext(AssetContext);
    ...
    const FormSchema = yup.object().shape({
        ...
        const handleSubmit = async (values) => {
          console.log("Assetvalues", values, userDetail?._id);
            try {
                if (selectedData) {                  
                  await updateAsset({selectedData: selectedData._id, registeringOfficer:userDetail?._id , values});
                }
                else {
                  // const values={...values, registeringOfficer:userDetail?._id}
                    console.log('@@@@@@@@@@@@@@@@@@@@@@@@', {...values, registeringOfficer:userDetail?._id});

                  await createAsset({...values, registeringOfficer:userDetail?._id});
                }
                setCreateOpen(false);
